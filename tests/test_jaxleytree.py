...
from neat import PhysTree, PhysNode, MorphTree, CompartmentFitter, CompartmentTree
...
class JaxleySimNode(PhysNode):
    ...
    def _make_branch(self, dx_max=15, factorlambda=1.0, pprint=False):
        if dx_max is None:
            n_comp = factorlambda
...
class JaxleySimTree(PhysTree):
    ...
    def init_model(
        ...
        # self.delete_model()
        # create the Jaxley
        cell = self._create_jaxley_cell(pprint=pprint)
        return cell

    def _create_jaxley_cell(self, pprint):
        # simple tree copy
        aux_tree = MorphTree(self)
        aux_tree.reset_indices()

        branches = [node._make_branch() for node in self]
